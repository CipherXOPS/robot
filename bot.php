	...
	function processMessage($message) {
		...
		if (isset($message['text'])) {
			...
			if ($text=="/start") {
				...
				apiRequest("sendMessage", array('chat_id' => $admin, "text" => "با استفاده از این بخش می توانید ربات خود را شخصی سازی #Source_Home کنید و بر پیام های دریافتی از سمت کاربران نظارت کنید\n🚫 = قفل شده\t\t✅ = آزاد","parse_mode" =>"MARKDOWN",'reply_markup' =>json_encode($keyboard) ));
				...
				apiRequest("sendChatAction",array('chat_id'=>$chat_id,"action"=>"typing"));
				apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "پیام خود را ارسال کنید . . . @Source_Home
